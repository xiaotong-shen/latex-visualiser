...
async function generateTikZFromClaude(selectedText: string): Promise<ClaudeVizResult> {
  ...
  const message = await client.messages.create({
    ...
    messages: [{
      role: 'user',
      content: `You are an expert mathematician and TikZ visualization specialist who creates publication-quality diagrams.
...
Requirements:
- Use the EXACT variable names, sets, and notation from the proof
- Draw a cross-section or schematic 2D diagram that represents the mathematical object
- Use filled regions with opacity for surfaces (\\fill[blue!15, opacity=0.5])
- Use ellipses to suggest 3D circular cross-sections where appropriate
- Label all key objects, points, boundaries, and parameters from the proof
- Use \\draw[->] for axes with proper mathematical labels
- Add dashed lines for hidden edges or reference lines
- Annotate with the parametrization or key equation in a boxed node
- Use color strategically: blue for main objects, red for special points, gray for reference
...
export function activate(context: vscode.ExtensionContext) {
    ...
    async () => {
      ...
      try {
        const result = await vscode.window.withProgress(
          { location: vscode.ProgressLocation.Notification, title: 'LaTeX Visualiser: Generating visualization suggestion', cancellable: false },
          async () => suggestionService.suggestVizBlock(contextData)
        );
        const ok = await editor.edit(eb => eb.insert(editor.selection.active, `\n${result.vizBlock}\n`));
        if (!ok) { vscode.window.showErrorMessage('Failed to insert generated viz block.'); return; }
        ...
        vscode.window.showErrorMessage(`Failed to generate suggestion: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  );
  ...
  const suggestOverlayCmd = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      try {
        const result = await vscode.window.withProgress(
          { location: vscode.ProgressLocation.Notification, title: 'LaTeX Visualiser: Generating overlay layers', cancellable: false },
        ...
        const ok = await editor.edit(eb => eb.insert(insertPosition, `\n${result.directives.join('\n')}`));
        if (!ok) { vscode.window.showErrorMessage('Failed to insert generated overlay directives.'); return; }
        if (result.usedFallback) {
        ...
        vscode.window.showErrorMessage(`Failed to generate overlay layers: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  );
  ...
  const editorChangeDisposable = vscode.window.onDidChangeActiveTextEditor(editor => {
    if (editor && isTexFile(editor.document)) { currentTexDocument = editor.document; }
  });
  ...
  context.subscriptions.push(
    openViewerCmd, refreshCmd, generateProofImageCmd,
    suggestVizCmd, suggestOverlayCmd,
    editorChangeDisposable, saveDisposable
  );
}
...
async function generateProofImage(context: vscode.ExtensionContext) {
  ...
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: '◈ LaTeX Visualiser: Generating proof image...', cancellable: false },
    async () => {
      try {
        ...
        if (currentChatPanel) {
          if (pngBase64) {
            ...
            currentChatPanel.webview.postMessage({ type: 'pdfError', text: 'Could not compile preview — check TikZ syntax' });
          }
        }
      } catch (err) {
...
function showChatPanel(preamble: string, figureCode: string, tikzCode: string, context: vscode.ExtensionContext) {
  const panel = vscode.window.createWebviewPanel(
    'latexProofChat', '◈ Proof Diagram Chat', vscode.ViewColumn.Beside, { enableScripts: true }
  );
  ...
  panel.webview.onDidReceiveMessage(async message => {
    if (message.type === 'openCodePanel') {
      showCodePanel(latestPreamble, wrapInFigure(message.tikz || latestTikz), context);
    }
...
% PREAMBLE: \\usetikzlibrary{...}`,
        ...
        if (preambleUpdateMatch) { latestPreamble = latestPreamble + '\n' + preambleUpdateMatch[1]; }
...
function showCodePanel(preamble: string, figureCode: string, context: vscode.ExtensionContext) {
  const panel = vscode.window.createWebviewPanel(
    'latexCodePanel', '◈ Image Code', vscode.ViewColumn.Beside, { enableScripts: true }
  );
...
function getCodePanelHtml(preamble: string, figureCode: string): string {
  ...
  return [
    ...
    '</head>',
    '<body>',
    '<div class="header"><h1>&#9672; Image Code</h1><p>Latest version of your generated TikZ diagram.</p></div>',
    '<div class="section"><div class="section-header"><span class="label">Preamble — add to top of your .tex file</span><button class="copy-btn" onclick="copySection(\'preamble\', this)">Copy</button></div><pre id="preamble">' + escapedPreamble + '</pre></div>',
    '<div class="section"><div class="section-header"><span class="label">TikZ Figure — paste where you want the image</span><button class="copy-btn" onclick="copySection(\'figure\', this)">Copy</button></div><pre id="figure">' + escapedFigure + '</pre></div>',
    '<button class="insert-btn" onclick="insertAtCursor()">Insert Figure After Selection in .tex File</button>',
    '<div class="toast" id="toast"></div>',
    '<script>',
    '  const vscode = acquireVsCodeApi();',
    '  function copySection(id, btn) { const text = document.getElementById(id).textContent; navigator.clipboard.writeText(text).then(function() { btn.textContent = "Copied!"; btn.classList.add("copied"); setTimeout(function() { btn.textContent = "Copy"; btn.classList.remove("copied"); }, 2000); }); }',
    '  function insertAtCursor() { vscode.postMessage({ type: "insertAtCursor" }); }',
    '  window.addEventListener("message", function(e) { const toast = document.getElementById("toast"); toast.textContent = e.data.text; toast.classList.add("show"); setTimeout(function() { toast.classList.remove("show"); }, 2500); });',
    '<\/script>',
    '</body></html>',
  ].join('\n');
}
...
async function openViewer(context: vscode.ExtensionContext) {
  ...
  if (currentPanel) {
    ...
    currentPanel = vscode.window.createWebviewPanel(
      'latexVisualiser', 'LaTeX Visualiser', vscode.ViewColumn.Beside,
      {
...
async function loadWebview(context: vscode.ExtensionContext, pdfPath: string | undefined) {
  ...
  currentPanel.webview.html = getWebviewContent({
    pdfBase64, pdfjsScriptUri, pdfjsWorkerUri, plotlyScriptUri,
    plots, markers,
